...
#ifndef LSMKV_KVSTORE_H
#define LSMKV_KVSTORE_H
#pragma once
...
public:
    ...
    explicit KVStore(const std::string &dir) : KVStoreAPI(dir) {
        stoDir = dir;
        time_flag_ = 0;
...
/* private function */
    ...
    SSTable *readSST(uint32_t level, Cache *cache) {
    ...
    void writeSST(uint32_t level, int file_name, SSTable *ssTable) {
    ...
    Cache *readCache(std::string &fileName) {
    ...
    SSTable *createSSTable() {
        ...
        connecter **mem_table_content = mem_table_.getWhole();
...
//    if(!overlapList.empty()) std::cout << "compaction" << std::endl;
            ...
            std::vector<connecter> entry_list;
